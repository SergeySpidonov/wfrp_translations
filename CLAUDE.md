...
### Этапы перевода
1. `scripts/extract.py` — извлекает текст постранично в `translations/pageXXX_en.txt`
2. Перевести страницу, следуя инструкциям ниже → сохранить в `translations/pageXXX_ru.txt`
...
При переводе любого игрового термина, названия, имени — строго следовать этому порядку:

1. **Студия 101** — если термин есть в глоссарии (колонка «Русский (студия 101)»), использовать его
2. **Мы** — если перевода студии 101 нет, но есть наш вариант (колонка «Русский (мы)»)
3. **Спросить пользователя** — если термин отсутствует в обоих столбцах

Глоссарий: `C:/Users/user/Desktop/ВФРП/Глоссарий.xlsx` (листы: «Игровая терминология», «ИменаНазвания»)
Извлечённый текстовый дамп: `C:/Users/user/Desktop/ВФРП/glossary_extracted.txt`
...
⚠️ Термины **подтверждены** из «Смерть на Рейке: дополнительные материалы (v1-0)».
